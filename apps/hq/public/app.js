...
vp.addEventListener('wheel',e=>{e.preventDefault();const oldZ=state.cam.zoom;const nz=Math.max(.3,Math.min(3,oldZ*(e.deltaY>0?.9:1.1)));const rect=vp.getBoundingClientRect();const mx=e.clientX-rect.left,my=e.clientY-rect.top;state.cam.x=mx-(mx-state.cam.x)*nz/oldZ;state.cam.y=my-(my-state.cam.y)*nz/oldZ;state.cam.zoom=nz;applyCam()},{passive:false});
$('zoom-in').addEventListener('click',()=>{state.cam.zoom=Math.min(3,state.cam.zoom*1.25);applyCam()});
$('zoom-out').addEventListener('click',()=>{state.cam.zoom=Math.max(.3,state.cam.zoom*.8);applyCam()});
$('zoom-fit').addEventListener('click',()=>{const f=$('floor');if(!f||!f.offsetWidth){return}const vw=vp.clientWidth,vh=vp.clientHeight;state.cam.zoom=Math.min(vw/f.offsetWidth,vh/f.offsetHeight,.95);state.cam.x=(vw-f.offsetWidth*state.cam.zoom)/2;state.cam.y=(vh-f.offsetHeight*state.cam.zoom)/2+20;applyCam()});
...
const HAIR={manager:'#5cb894','marketing-agent':'#dfa45a','designer-agent':'#af97d8','prospector-agent':'#82c99a','research-agent':'#c9b8db'};
...
function renderFloor(data){
  ...
  for(const dept of data.office.departments){
    ...
    h+=`<div class="room ${doorCls}" style="left:${a.x*CELL}px;top:${a.y*CELL}px;width:${a.w*CELL}px;height:${a.h*CELL}px;${doorVar}"><div class="room-label">${esc(dept.label)}</div></div>`;
    for(const[aid,pos]of Object.entries(dept.desks)){
      h+=`<div class="desk-chair" style="left:${pos.x-11}px;top:${pos.y+16}px"></div>`;
      h+=`<div class="desk-item" style="left:${pos.x-29}px;top:${pos.y-17}px"><div class="desk-monitor" data-mon="${esc(aid)}"></div></div>`;
    }
  }
  // Decorations
  h+=`<div class="decor plant" style="right:30px;bottom:40px"></div><div class="decor plant" style="right:60px;bottom:40px"></div>`;
  h+=`<div class="decor sofa" style="left:300px;bottom:30px"></div>`;
  h+=`<div class="decor whiteboard" style="left:340px;top:80px"></div>`;
  for(const ag of data.agents){
    const id=String(ag.id),pos=ag.position;if(!pos)continue;
    state.agentsById[id]=ag;
    const color=COLORS[id]||'#999';const hair=HAIR[id]||color;
    const cls=stCls(ag.status);
    h+=`<div class="agent ${cls}" id="agent-${esc(id)}" style="left:${pos.x}px;top:${pos.y-46}px" data-agent="${esc(id)}" title="${esc(ag.name)} — ${esc(stPt(ag.status))}"><span class="nametag">${esc(ag.name)}</span><div class="hair" style="background:${hair}"></div><div class="head"></div><div class="body" style="background:${color}"></div><div class="sdot"></div></div>`;
  }
  f.innerHTML=h;
...
function animateMove(d){
  ...
  el.style.left=target.position.x+'px';el.style.top=(target.position.y-46)+'px';
...
function render(data){
  ...
  $('board').innerHTML=Object.entries(cols).map(([label,st])=>{
    ...
    return `<div class="b-col"><h5>${label}</h5>${items.map(t=>`<div class="b-card"><b>${esc(t.title)}</b><small>${esc(t.assigned_agent||'—')} · ${esc(stPt(t.status))}</small></div>`).join('')}</div>`}).join('');
...
async function openProfile(aid){
  ...
  try{
    ...
    $('profile-content').innerHTML=`
      ...
      <div class="p-sec"><h4>Tarefas</h4>${(p.tasks??[]).map(t=>`<div class="p-item"><b>${esc(t.title)}</b><small>${esc(stPt(t.status))}${t.completed_at?' · '+t.completed_at.slice(0,16):''}</small></div>`).join('')||'<p class="muted">Nenhuma.</p>'}</div>
      ...
      <div class="p-sec"><h4>Runs</h4>${(p.runs??[]).map(r=>`<div class="p-item">${esc(r.id.slice(0,22))}<small>${esc(r.state)} · etapa ${r.current_step} · ${r.retry_count} tentativas</small></div>`).join('')||'<p class="muted">Nenhum.</p>'}</div>`;
...
$('btn-send').addEventListener('click',async()=>{
  ...
  try{
    ...
    if(r.ok){toast('✓ Executado');await refresh()}
...
$('btn-pause').addEventListener('click',async()=>{
  try{addMsg('pare tudo','user');const r=await(await fetch(`${API}/api/hq/command`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text:'pare tudo'})})).json();addMsg(r.message,'mgr');await refresh()}catch{toast('Erro.')}
});
$('btn-resume').addEventListener('click',async()=>{
  try{addMsg('continue','user');const r=await(await fetch(`${API}/api/hq/command`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text:'continue'})})).json();addMsg(r.message,'mgr');await refresh()}catch{toast('Erro.')}
});
...
$('btn-voice').addEventListener('click',async()=>{
  ...
  try{
    ...
    recorder.onstop=async()=>{stream.getTracks().forEach(t=>t.stop());$('btn-voice').textContent='🎙️ Falar';
